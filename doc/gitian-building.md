...
*Setup instructions for a Gitian build of Invictus using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Invictus
...
- [Building Invictus](#building-invictus)
...
Clone the git repositories for invictus and Gitian and then checkout the invictus version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/invictus/invictus.git
cd invictus
...
Follow the instructions in [doc/release-process.md](release-process.md) in the invictus repository
...
To build Invictus (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the invictus repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/invictus/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/invictus/invictus
...
For example:
```bash
URL=https://github.com/user/invictus.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit invictus=${COMMIT} --url invictus=${URL} ../invictus/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit invictus=${COMMIT} --url invictus=${URL} ../invictus/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit invictus=${COMMIT} --url invictus=${URL} ../invictus/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/invictus-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/invictus-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/invictus-build.assert
```
...
[invictus/gitian.sigs](https://github.com/invictus/gitian.sigs.phils/) repository, or if that's not possible create a pull
request. 
